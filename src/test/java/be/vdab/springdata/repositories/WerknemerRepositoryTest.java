...
class WerknemerRepositoryTest extends AbstractTransactionalJUnit4SpringContextTests {
    ...
    void findByFiliaalGemeente() {
        ...
        assertThat(repository.findByFiliaalGemeente(antwerpen)).hasSize(countRowsInTableWhere(WERKNEMERS, "filiaalId = (select id from filialen where gemeente = 'Antwerpen')")).first().extracting(Werknemer::getFiliaal).extracting(Filiaal::getGemeente).isEqualTo(antwerpen);
    }
    ...
    void findByVoornaamStartingWith() {
        assertThat(repository.findByVoornaamStartingWith("J"))
                ...
                .extracting(Werknemer::getFiliaal).extracting(Filiaal::getNaam);
    }
    @Test
    void eerstePagina() {
